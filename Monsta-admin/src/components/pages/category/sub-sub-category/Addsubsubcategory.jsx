...
export default function Addsubsubcategory() {
  ...
  const navigate = useNavigate();
   ...
   const handleSubmit=(event)=>{
    ...
    if (updateId) {
      var url= import.meta.env.VITE_API_BASE_URL + import.meta.env.VITE_SUB_SUB_CATEGORY_UPDATE + '/' + updateId;
      axios.put(url, data)
        .then((success) => {
      ...
      var url = import.meta.env.VITE_API_BASE_URL + import.meta.env.VITE_SUB_SUB_CATEGORY_CREATE;
      axios.post(url, data)
        .then((success) => {
          if (success.data._status == true) {
            toast.success(success.data._message);
  ...
  useEffect(()=>{
    if(updateId){
        axios.post(`http://localhost:8000/api/admin/sub-sub-categories/details`,{
          id:updateId
        })
        .then((response)=>{
           if(response.data._status==true){
      ...
      <div className="w-full min-h-[610px]">
        <div className="max-w-[90%] mx-auto py-5">
          <div className="flex items-center justify-between bg-slate-100 py-3 px-4 rounded-t-md border border-slate-400">
            <h3 className="text-[22px] font-semibold">{updateId ? 'Update Sub Sub Category':'Add Sub Sub Category'}</h3>
          </div>

          <div className="border border-t-0 rounded-b-md border-slate-400 p-4 space-y-4">
            <form onSubmit={handleSubmit}>
              <div><label class="block mb-1 font-medium">Sub Sub Category Image</label>
                <input
                  type="file"
                  name="image"
                  accept="image/*"
              ...
              <div><label class="block mb-1 font-medium my-3">Parent category</label>
                <select class="w-full border border-gray-300 p-2 rounded" placeholder="Enter Category Name" name='parent_category_id'>
                  ...
                  {Category.map((items, index) => {
                    return (
                      <option key={index} value={items._id} selected={items._id==SubsubcategoryDetails.parent_category_id}>{items.name}</option>
                    )
                  })}
                </select>
              ...
              <div><label class="block mb-1 font-medium my-3">Sub category</label>
                <select class="w-full border border-gray-300 p-2 rounded" placeholder="Enter Category Name" name='sub_category_id'>
                  <option value="">select Sub category</option>
                  {subCategory.map((items, index) => {
                    return (
                      <option key={index} value={items._id} selected={items._id==SubsubcategoryDetails.sub_category_id}>{items.name}</option>
                    )
                  })}
                </select>
              ...
              <div><label class="block mb-1 font-medium my-3">Sub Category Name</label><input class="w-full border border-gray-300 p-2 rounded" placeholder="Enter SubCategory Name" type="text" name='name' defaultValue={SubsubcategoryDetails.name}/></div>

              <div><label class="block mb-3 font-medium">Order</label><input class="w-full border border-gray-300 p-2 rounded" placeholder="Enter Order" type="number" name='order' defaultValue={SubsubcategoryDetails.order} /></div>

              {/* Submit Button */}
              <button className="bg-indigo-600 text-white px-4 py-2  mt-2 rounded hover:bg-indigo-700">
                {updateId ? 'Update Sub Sub Category':'Add Sub Sub Category'}
              </button>
            </form>
