...
export default function ViewMaterial() {
   ...
   }, [Material, currentPage, nameFilter, apiStatus])
   ...
   let checkedAll = () => {
      if (Material.length != checkedValue.length) {
         var data = [];
         Material.forEach((values) => {
   ...
   const changeStatus = () => {
      if (checkedValue.length > 0) {
         axios.put(import.meta.env.VITE_API_BASE_URL + import.meta.env.VITE_MATERIAL_CHANGE_STATUS, {
            id: checkedValue
         })
            .then((response) => {
               if (response.data._status == true) {
   ...
   const deleteRecords = () => {
      if (checkedValue.length > 0) {
         if (confirm('Are you sure you want to delete')) {
            axios.put(import.meta.env.VITE_API_BASE_URL + import.meta.env.VITE_MATERIAL_DELETE, {
               id: checkedValue
            })
               .then((response) => {
                  if (response.data._status == true) {
   ...
   return (
      <section className="w-[100%]  place-items-center mx-auto">
         <Breadcrumb parent="Home" parent_link="/" child="Material" child_link="/material/view" page_name="view" />
         ...
         {Material.length == 0 ? (
            ...
            <div class="w-full ">
               <div class="max-w-[95%] mx-auto py-5">
                  <div class="flex items-center justify-between bg-slate-100 py-3 px-4 rounded-t-md border border-slate-400">
                     <h3 class="text-[22px] font-semibold">View Materials</h3>
                           ...
                           <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                              <tr>
                                 <th class="px-4 py-3"><input onClick={checkedAll}
                                    checked={Material.length == checkedValue.length ? 'checked' : ''} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" type="checkbox" /><label for="checkbox-all-search" class="sr-only">checkbox</label></th>
                              ...
                              {Material.map((items, index) => {
                                 return (
                                    <tr class="bg-white border-b hover:bg-gray-50">
                                       ...
                                       <td class="px-4 py-3"><a href={`/material/update/${items._id}`} data-discover="true"><button class="text-blue-600 hover:underline text-sm">Edit</button></a></td>
                                    </tr>
                                 )
                              })}
