...
export default function Enquiry() {
   ...
   useEffect(() => {
      axios.post(import.meta.env.VITE_API_BASE_URL + import.meta.env.VITE_CONTACT_VIEW, {
         page: currentPage,
         limit: 10,
      })
         .then((result) => {
   ...
   return (
      <section className="w-[100%]  place-items-center mx-auto">
         <Breadcrumb parent="Home" parent_link="/" child="contact" child_link="/enquiry" page_name="view" />
         {user.length === 0 ? (
               ...
               <div className="w-full min-h-[fit-content]">
                  <div className="max-w-[90%] mx-auto py-5">
                     <div className="flex item-center justify-between bg-slate-100 py-3 px-4 rounded-t-md border border-slate-400">
                        <h3 className="text-[22px] font-semibold">View Contact</h3>
                              ...
                              <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                 <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                    <tr>
                                       ...
                                       <th scope="col" className="w-[11%]">Subject</th>
                                       <th scope="col" className="w-[11%]">message</th>
                                    ...
                                    {user.map((items, index) => {
                                       return (
                                          <tr key={index} className="bg-white  dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                                             ...
                                             <td className="py-4"> {items.subject}</td>
                                             <td className="py-4"> {items.message}</td>
                                             {/*                               
                                 <td className=" py-4">
                                    <Link to="/user">
