...
export default function ViewTestimonial() {
  ...
  useEffect(() => {
    axios.post(import.meta.env.VITE_API_BASE_URL + import.meta.env.VITE_TESTIMONIAL_VIEW, {
      page: currentPage,
      limit: 1,
    })
      .then((result) => {
  ...
  const changeStatus = () => {
    if (checkedValue.length > 0) {
      axios.put(import.meta.env.VITE_API_BASE_URL + import.meta.env.VITE_TESTIMONIAL_CHANGE_STATUS, {
        id: checkedValue
      })
        .then((response) => {
  ...
  const deleteRecords = () => {
    if (checkedValue.length > 0) {
      if (confirm('Are you sure you want to delete')) {
        axios.put(import.meta.env.VITE_API_BASE_URL + import.meta.env.VITE_TESTIMONIAL_DELETE, {
          id: checkedValue
        })
          .then((response) => {
            if (response.data._status == true) {
        ...
        <div className="w-full min-h-[fit-content]">
          <div className="max-w-[90%] mx-auto py-5">
            <div className="flex items-center justify-between bg-slate-100 py-3 px-4 rounded-t-md border border-slate-400">
              <h3 className="text-[22px] font-semibold">View Testimonials</h3>
              ...
              <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table className="w-full text-sm text-left text-gray-500">
                  <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                    <tr>
                      <th><input onClick={checkedAll}
                        checked={testimonials.length == checkedValue.length ? 'checked' : ''} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" type="checkbox" /><label for="checkbox-all-search" class="sr-only">checkbox</label></th>
