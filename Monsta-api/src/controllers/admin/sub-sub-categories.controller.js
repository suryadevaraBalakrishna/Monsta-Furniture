...
exports.create = async (request, response) => {
    ...
    if(request.file){
        ...
        const insertData = new subsubcategoriesModal(data);
...
exports.view = async (request, response) => {
        ...
        if(request.body.name != undefined){
         ...
         if(request.body.parent_category_id != undefined){
            if(request.body.parent_category_id != ''){
                var parent_category_id = new RegExp(request.body.parent_category_id,"i");
                addCondition.push({ parent_category_id : parent_category_id })
            }
        }

        if(request.body.sub_category_id != undefined){
            if(request.body.sub_category_id != ''){
                var sub_category_id = new RegExp(request.body.sub_category_id,"i");
                addCondition.push({ sub_category_id : sub_category_id })
            }
        }
    }
    ...
    var totalRecords=await subsubcategoriesModal.find(filter).countDocuments();
    ...
    await subsubcategoriesModal.find(filter)
        ...
        .then((result) => {
            if (result.length > 0) {
                const output = {
                    ...
                    _image_path:process.env.subsubcategory_image,
                    _data: result
                }
...
exports.details = async (request, response) => {
    ...
    // })
    try{
    await subsubcategoriesModal.findById(request.body.id)
        .then((result) => {
            if (result) {
                const output = {
                    _status: true,
...
exports.update = async (request, response) => {
    ...
    //     name: request.body.name,
    //     order: request.body.order,
    //     parent_category_id:request.body.subcategory_image
    // }
    ...
    await subsubcategoriesModal.updateOne({
...
exports.changeStatus = async (request, response) => {
       ...
       await subsubcategoriesModal.updateMany({
           _id: {
               $in: request.body.id
           }
       },[ {
